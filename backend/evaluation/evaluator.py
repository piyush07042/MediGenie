...
def load_disease_dataset(disease_key: str) -> Tuple[pd.DataFrame, pd.Series, Dict[str, Any]]:
    ...
    model_dir = MODELS_DIR / info["model_folder"]
    ...
    if disease_key == "heart_disease":
        ...
        df["insulin"] = df["insulin"].apply(lambda x: 0.0 if str(x).lower() == "no" else 85.0)
        df["bmi"] = 28.5
        df["glucose"] = 115.0
        ...
        with open(raw_path, "r", encoding="utf-8", errors="ignore") as fh:
            for line in fh:
                ...
                if line_str.startswith("@attribute"):
                    ...
                    vals = line_str.split(",")
                    # Truncate ragged rows to match attribute count
                    rows.append(vals[:len(attrs)])

        df = pd.DataFrame(rows, columns=attrs)
        df["target"] = df["class"].astype(str).apply(lambda x: 1 if "ckd" in x.lower() else 0)
        df["age"] = pd.to_numeric(df["age"].replace("?", np.nan), errors="coerce").fillna(45.0)
        df["creatinine"] = pd.to_numeric(df["sc"].replace("?", np.nan), errors="coerce").fillna(1.2)
        df["blood_urea"] = pd.to_numeric(df["bu"].replace("?", np.nan), errors="coerce").fillna(30.0)
        df["albumin"] = pd.to_numeric(df["al"].replace("?", np.nan), errors="coerce").fillna(0.0)
        df["sgpt"] = 35.0
        ...
        for col in ["age", "bilirubin", "alk_phosphatase", "sgpt", "sgot"]:
            df[col] = pd.to_numeric(df[col], errors="coerce").fillna(df[col].mean() if df[col].mean() is not np.nan else 1.0)
        ...
        df = pd.read_csv(raw_path, header=None)
        # UCI Hepatitis format
        df["target"] = (df[0].astype(str) == "1").astype(int)
        df["age"] = pd.to_numeric(df[1].replace("?", np.nan), errors="coerce").fillna(40.0)
        df["bilirubin"] = pd.to_numeric(df[14].replace("?", np.nan), errors="coerce").fillna(1.0)
        df["alk_phosphatase"] = pd.to_numeric(df[15].replace("?", np.nan), errors="coerce").fillna(85.0)
        df["sgot"] = pd.to_numeric(df[16].replace("?", np.nan), errors="coerce").fillna(35.0)
        df["sgpt"] = df["sgot"]
