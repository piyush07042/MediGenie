...
def extract_patient_metrics(text: str) -> dict:
    ...
    pid_match = re.search(r'(?:Patient\s*ID|PID|ID)[\s:-]*([A-Z0-9-]+)', text, re.IGNORECASE)
    if pid_match:
    ...
    gender_match = re.search(r'(?:Gender|Sex)[\s:-]*(Male|Female)', text, re.IGNORECASE)
    if gender_match:
        metrics["gender"] = gender_match.group(1).capitalize()

    glucose_match = re.search(r'(?:Fasting\s*Glucose|Glucose|Blood\s*Sugar)[\s:-]*(\d+(?:\.\d+)?)', text, re.IGNORECASE)
    if glucose_match:
    ...
    bp_match = re.search(r'(?:Systolic\s*BP|Blood\s*Pressure|BP)[\s:-]*(\d{2,3})', text, re.IGNORECASE)
    if bp_match:
        metrics["systolic_bp"] = float(bp_match.group(1))

    chol_match = re.search(r'(?:Cholesterol|Total\s*Cholesterol)[\s:-]*(\d+(?:\.\d+)?)', text, re.IGNORECASE)
    if chol_match:
