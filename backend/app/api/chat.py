...
def _build_rag_sources(final_state: AgentState) -> list[str]:
    """Collect source references from RAG knowledge results."""
    sources: list[str] = []
...
async def chat(
    ...
    try:
        ...
        rag_sources = _build_rag_sources(final_state)
...
async def chat_stream(
    ...
    try:
        ...
        async def event_generator(text: str):
            ...
            yield f"data: {json.dumps({'type': 'done', 'clinical_summary': clinical_summary, 'sources': rag_sources, 'follow_up_suggestions': follow_up_suggestions, 'clinical_intelligence': clinical_intelligence})}\n\n"
