...
def get_profile(current_user: User = Depends(get_current_user)):
    return ApiResponse(message="Profile retrieved", data={"id": current_user.id, "email": current_user.email, "full_name": current_user.full_name})
...
def upload_avatar(file: UploadFile = File(...), db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    # store avatar url on patient if exists or on user via additional field (simpler: return path)
