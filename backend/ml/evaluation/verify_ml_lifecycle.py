...
from ml.evaluation.train_and_evaluate_all_models import generate_clinical_dataset
...
def run_evaluation(cfg: dict, predictor: Predictor) -> dict[str, float | int]:
    ...
    df = generate_clinical_dataset(cfg["key"], cfg["n_samples"])
    X = df[cfg["features"]]
    y = df["target"].values
...
def verify_ml_lifecycle():
    ...
    for cfg in MODELS_CONFIG:
        ...
        df = generate_clinical_dataset(key, cfg["n_samples"])
